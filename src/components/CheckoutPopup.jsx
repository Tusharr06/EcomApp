            ...
            {cartItems.map((item) => (
              <div key={item.id} className="checkout-item" style={{ display: 'flex', marginBottom: '10px' }}>
                ...
                <div>
                  ...
                  <p>Price: ₹{item.price * item.quantity}</p> {/* Changed to rupees */}
                </div>
              </div>
            ))}
            <div>
              <h5 
                style={{color: 'black', display: 'flex', justifyContent: 'center', fontSize: '1.3rem', fontWeight: 'bold'}} 
              >
                Total: ₹{totalPrice} {/* Changed to rupees */}
              </h5>
            </div>
          </div>
        </Modal.Body>
